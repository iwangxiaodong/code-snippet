...
import static org.jitsi.utils.MediaType.AUDIO;
import static org.jitsi.utils.MediaType.VIDEO;
...
public class AVReceive2 {
    ...
    private boolean initialize()
         ...
         */
        MediaType[] mediaTypes = {MediaType.AUDIO}; //MediaType.values();
...
//            MediaStream mediaStream = mediaService.createMediaStream(device);
            ...
            var cdi = ((MediaDeviceImpl) mediaService.getDefaultDevice(MediaType.AUDIO, MediaUseCase.CALL)).getCaptureDeviceInfo();
            if (cdi == null) { // 因getDefaultDevice匹配不到设备会返回一个非null实例，故只能用getCaptureDeviceInfo()来判null。
                var as = AudioSystem.getAudioSystem(LOCATOR_PROTOCOL_PORTAUDIO); // 换成不支持 IS_ANDROID 的 PortAudioSystem。
                var devices = as.getDevices(AudioSystem.DataFlow.CAPTURE);
                System.out.println(devices);
                if (!devices.isEmpty()) {
                    var cd = (CaptureDevice) Manager.createDataSource(devices.getFirst().getLocator());
                    cdi = cd.getCaptureDeviceInfo();
                }
            }
            if (cdi != null) {
                device = new MediaDeviceImpl(cdi, MediaType.AUDIO);
                mediaStream = mediaService.createMediaStream(device);
                System.out.println("mediaStream - " + mediaStream);
            }
